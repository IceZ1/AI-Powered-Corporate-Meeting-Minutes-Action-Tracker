...
class Meeting(db.Model):
    ...
    Attributes:
        ...
        action_items: Action items in JSON format (list of tasks with owner and deadline)
    ...
    def __init__(self, title, summary, action_items, decisions, original_transcript):
        ...
        self.action_items = json.dumps(action_items) if isinstance(action_items, list) else action_items
        self.decisions = json.dumps(decisions) if isinstance(decisions, list) else decisions
    ...
    def __repr__(self):
        ...
        return f'<Meeting {self.title} - {self.date_created.strftime("%Y-%m-%d %H:%M")}>'