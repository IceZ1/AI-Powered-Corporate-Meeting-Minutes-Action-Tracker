...
def process_meeting_transcript(transcript_text):
    ...
    response = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": "You are an AI assistant that processes meeting transcripts..."},
            {"role": "user", "content": f"Process this meeting transcript: {transcript_text}"}
        ],
        temperature=0.3
    )
    """
...
def generate_summary_points(transcript):
    ...
    summary_templates = [
        ...
        f"Reviewed current status of {keywords[1] if len(keywords) > 1 else 'ongoing projects'}",
        f"Addressed key challenges related to {keywords[2] if len(keywords) > 2 else 'project execution'}",
...
def generate_action_items(transcript):
    ...
    # Look for action-oriented language in transcript
    action_verbs = ["implement", "develop", "create", "review", "analyze", "prepare", "schedule", "follow up"]
    ...
    for i, action in enumerate(sample_actions[:3]):  # Limit to 3 action items
        deadline = (datetime.now() + timedelta(days=random.randint(3, 14))).strftime("%Y-%m-%d")
        owner = team_members[i % len(team_members)] if team_members else f"Team Member {i+1}"
...
def extract_keywords(text):
    ...
    default_keywords = ["project", "development", "strategy", "implementation", "review", "planning"]
    ...
    stop_words = {"that", "this", "with", "from", "they", "were", "been", "have", "will", "would", "could", "should"}
